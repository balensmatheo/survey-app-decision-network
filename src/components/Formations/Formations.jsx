...
function Formations(props) {
    ...
    return(
        <div className={"formations"}>
            <Heading className={"welcome"} marginTop={"2em"} level={4}>Bonjour {props.user.attributes.email}, voici les formations disponibles</Heading>
                        ...
                        <Card className={"cards"} marginBottom={"2em"} key={index} padding={"1rem"}>
                            ...
                            >
                                ...
                                >
                                    ...
                                    <Flex flex={"1 1 auto"} direction={"row"} alignItems={"flex-end"} justifyContent={"flex-end"}>
                                        <Badge borderRadius={"2px"} size={"small"}>Formateur : {item.formateur}</Badge>
                                    </Flex>
                                </Flex>
                                ...
                                <Flex
                                    ...
                                    alignItems={"center"}
                                >
                                    <Flex marginLeft={"1em"} direction={"row"} width={"100%"} justifyContent={"flex-start"} alignItems={"center"}>
                                        <Badge size={"small"} variation={"info"}>{item.date}</Badge>
                                        <Badge className={"formation-length"} size={"small"} variation={"success"}>{getDuree(item.date, item.date_fin)} jours</Badge>
                                    </Flex>
                                    <Flex direction={"column"}  alignItems={"center"} justifyContent={"center"}>
                                        {
                                            item.participants.find(participant=>participant===props.user.attributes.email) ?
                                                <Button size={"small"} className={"continue-form"} onClick={() => navigateToForm(item)}>Continuer</Button>
                                                :
                                                <Button  className={"subscribe-btn"} isDisabled={item.participants.find(participant=> participant===props.user.attributes.email)} onClick={() => subscribe(item.id, item)}>S'inscrire</Button>
                                        }
                                    </Flex>
                                </Flex>
                            </Flex>
